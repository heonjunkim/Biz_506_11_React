...
class BucketMain extends Component {
   ...
   */
  // 화면이 이미 완성된 상태에서
  // 데이터가 변화했을때 rerendering하려고 할때 실행되는 method
  // insert, update, delete 되었을때 실행될 method
  ...
  componentDidMount() {
    ...
    if (loadBucketList) {
      ...
      this.setState({ bucketList: JSON.parse(loadBucketList) });
  ...
  /**
   * class Component에서 child Comopnent에 state형 변수를 보낼떄는
   * 보낼변수명={this.state.변수}
   *
   * 함수를 보낼떄
   * 보낼 함수명= {this.함수명}
