...
class TodoMain extends Component {
  ...
  state = {
    todoList: [
      ...
      { id: 2, text: "TODO List 만드는 날", isComplete: false },
    ],
  };
  /**
   ...
   */
  onToggle = (id) => {
    // id값을 기준으로 todoList 변수의 isComplete을 변경
    ...
    /**
     * 클래스 방식 Component에서는 state 변수를 setting 하기 위한 setter를
     * 별도로 만들지 않는다
     *
     * state 변수를 setting하기 위해서는
     * this.setState() 라는 공통함수를 사용한다
     *
     * this.setState({state변수 : 새로운값} )형식으로 setting 한다
     */
  ...
  //state형 쓸대는 id: this.id++ 이렇게 쓸수없다
  onDeleteItem = (id) => {
...
// props를 통해서 받은 함수는 this를 붙이지 않는다
