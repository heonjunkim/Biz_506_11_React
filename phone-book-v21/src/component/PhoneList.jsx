...
const PhoneList = ({
   ...
   * 즉, action.type값을 임의로 정하여 reducer함수가 작동되는 방법을 정의할 수 있다
   *
   */
  const reducer = (object, action) => {
    // 만약 action.type 이 EDIT_form으로 설정되어서 reducer가 호출되면
    // 실행될 코드
    if (action.type === "EDIT_FORM") {
      return { name: action.name, number: action.number };
    }
    // input box에 문자열을 입력하는 change 이벤트가 발생할때 default로 실행될 코드
