...
const PhoneInsert = ({ onClickSave }) => {
  ...
  사용자가 키보드로 입력한 문자열을 state 변수에 저장해주는 코드를 작성해야한다

  input box에 클릭을하고 키보드로 문자를 입력하면 한글자마다 (e) 매개변수의 
  e.target.value값에 일단 저장이 된다
  e.target.value값에 저장된 문자열을 setName() 함수를 통하여 name state 변수에
  저장을 해준다  
  */
  const nameChange = (e) => {
  ...
  이벤트 핸들러에 어떤값을 전달하여 그 값으로 무언가 연산을 수행하도록 하려면
  ({전달하고자 하는 변수}) => onClick(전달하고자하는 변수) 형식으로 사용해야 한다
